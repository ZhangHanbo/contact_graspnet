...
def inference(
        ...
        checkpoint_dir, 
        input_paths, 
        output_paths=os.path.join(file_dir, '../../demos/outputs'),
        K=None, 
        local_regions=True, 
    ...
    for p in glob.glob(input_paths):
        ...
        # Save results
        np.savez(os.path.join(output_paths, 'predictions_{}'.format(os.path.basename(p.replace('png','npz').replace('npy','npz')))), 
